...
class Case1Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.addButton.setOnClickListener {
            quantity++
            binding.quantityText.text = "$quantity"
            it.announceForAccessibility(getString(R.string.add_quantity_annonce) + "$quantity")
        }
        ...
        binding.removeButton.setOnClickListener {
            if (quantity > 0) {
                quantity--
                binding.quantityText.text = "$quantity"
                it.announceForAccessibility(getString(R.string.remove_quantity_annonce) + "$quantity")
            } else {
